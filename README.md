...
![](assets/pictures/hashtable_example.jpg)
...
Для оценки распределения хеш-функций программа запускается на наборе слов из файла [words.txt](assets/data/words.txt). В файле находится 58111 различных слов. Работа хеш-функции оценивается на нескольких размерах таблицы: 751, 2017, 4007, 8009. 

Это простые числа. Это необязательное требование к размеру таблицы, но обязательное для функций, обладающих свойством периодичности. Так мы уменьшаем вероятность скопления элементов по значениям хеша, кратным этому периоду, и, как следствие, улучшаем итоговое распределение функции.

Размеры подобраны таким образом, чтобы отследить поведение функций в различных ситуациях и выделить у них, например, такие свойства, как ограниченность. Полученные размеры списков записываются в dat файлы, по которым строятся гистограммы с помощью gnuplot.

### Ускорение поиска <a name="ускор-поиска" />

Все программы компилируются с флагом `-O2`.

Тест заключается в поиске случайных слов из файла. Используется тот же файл, что и для оценки распределения хеш-функций. Поиск запускается 20 млн раз. 

*Примечание*: при подготовке к тестированию слова вносятся в хеш-таблицу с учётом операции рехеширования, чтобы использовать оптимальные длины списков (в среднем не более 5 элементов). Чтобы не учитывать влияние ввода слов, поиск запускается 20 млн раз. Так можно пренебречь временем вставки слов в таблицу и учитывать именно время поиска.
...
10. Проведён ряд оптимизацией, подробнее о которых сказано в [соответствующем разделе](#рез-ускор)
...
## Ускорение таблиц <a name="рез-ускор" />
...
**Время работы:** $1.066 \pm 0.005$ сек
...
**Время работы:** $0.911 \pm 0.006$ сек
...
По результатам профилирования **версии №2** в топ снова вышла функция поиска, но, к сожалению, оптимизировать её не представляется возможным, так как в ней находится просто вызов расчёт хеша и цикл, пробегающийся по соответствующему списки. Было решено проанализировать ещё раз работу хеш-функции.

Ранее мы решили считать её по блокам, но перед этим мы считали её длины. Было выдвинуто предположение, что нет смысла считать её длины и считать хеш именно этого фрагмента строки. 

В рамках первой оптимизации мы упаковали все строки по 32 символа, поэтому теперь можно считать хеш от всей этой строки (4 блока по 8 символов), не пренебрегая нулевыми символами в конце.

Оптимизация оформлена в виде ассемблерной вставки:
```
        asm
        (
...
**Время работы:** $0.432\pm 0.003$ сек
...
В таблице приведено время работы в секундах. 
- Абсолютное ускорение - ускорение каждой из версий относительно базовой
- Относительное ускорение - ускорение каждой из версий относительно предыдущей

|                | Время работы      | Абсолютное ускорение | Относительное ускорение |
|----------------|-------------------|----------------------|-------------------------|
| Базовая версия | $2.42 \pm 0.02$   | $1$                  |                         |
| Версия №1      | $1.066 \pm 0.005$ | $2.270 \pm 0.008$    | $2.270 \pm 0.008$       |
| Версия №2      | $0.911 \pm 0.006$ | $2.656 \pm 0.004$    | $1.170 \pm 0.002$       |
| Версия №3      | $0.432 \pm 0.003$ | $5.602 \pm 0.007$    | $2.109 \pm 0.008$       |

# Вывод

Не всегда флагов компилятора достаточно для получения максимального ускорения. Для оптимизации программы следует учитывать область её использования. За счёт этого было достигнуто ускорение почти в 5.6 раз